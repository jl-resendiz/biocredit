...
Existing diagnoses point at metric design, monitoring gaps, and political
compromise~\cite{Maron2016}, but whether the market design itself
systematically excludes most ecosystem types from receiving investment
...
interact across these credit types. We adapt the market-ecology framework
of Scholl, Calinescu and Farmer~\cite{SchollCalinescuFarmer2021} to a
compliance market: species are defined by behavioural strategy rather
than by institutional identity:
(i) cost-minimising compliance buyers: agents seeking the cheapest credit
...
Third, credit types with ten or more prior transactions are approximately 8.4
times more likely to be chosen than untraded types (intermediate transaction
counts receive smaller weights, ranging from 1.5$\times$ to 5.1$\times$;
Methods), creating a self-reinforcing cycle in which popular types
...
Buyer-choice flexibility erodes the scarcity signal that should
incentivise impact avoidance~\cite{zuErmgassen2020}; the procurement
flexibility mandate avoids that failure mode. It does not let
individual buyers substitute across types; it reallocates a fixed
share of obligations to underserved types under regulator direction,
trading type-specific compensation for coverage breadth on the
redirected portion while preserving like-for-like on the rest.
...
least-cost compliance is likely to concentrate demand before coverage
...
behalf~\cite{IPART2024}. Of the 24 credit categories served only by the
Trust, each has received at least one purchase that the IPART transaction
record classifies as a Fund-routed acquittal rather than the Trust's
own-budget conservation spending. This is consistent with developer demand
existing for these types but being routed away from the open market via
the Fund.
...
The discretion is associated with a measurable price differential: in
thin markets (fewer than five lifetime transactions), the Trust pays a
median of AUD\,5,989 per credit versus AUD\,1,125 for direct compliance
buyers, a 5.3-fold gap that is highly significant (Methods).
...
Trust (65.9\%/34.1\%) reproduces the observed 65\%/35\% breakdown. Five
...
per transaction. Of the five formations that are near-extinct in the data,
four (Grasslands, Rainforests, Saline Wetlands, Heathlands) recover under
at least one demand-redirecting intervention, while one (Arid Shrublands,
Acacia sub-formation) remains market-extinct under all scenarios: its
development pressure is so low that no demand-side intervention generates
obligations there.
...
government fund.
The NSW regulator reached the same diagnosis on different
grounds~\cite{IPART2023}, recommending that the Fund pay-in option be
phased out. The model supplies the missing mechanism: every
obligation that enters the Fund is one fewer direct transaction in
the open market, and the cumulative effect is a 13 percentage point
reduction in Fund routing (from 34.1\% to 21.3\%) when bypass is
restricted.

Bypass reduction in the model operates as a necessary precondition rather
than a stand-alone fix. The empirical record sits on a different margin:
the 2024 Amendment Act acquittal deadline forced developers to acquit dormant
obligations, lowering the unacquitted-obligation share from 81\% in
2022--23 to approximately 18\% in 2024--25~\cite{IPART2025}, while
functional coverage held at 18\% over the same window (Supplementary
Table~1 footnote). Acquittal enforcement (the empirical lever) and
within-formation substitution (the model parameter) are distinct
mechanisms; that coverage did not move when obligations were forced to
acquit is consistent with the model's finding that bypass reduction alone,
without procurement flexibility to redirect freed-up demand toward
underserved types, yields limited coverage gains.

The US 2008 Final Rule codified the same hierarchy for wetland
mitigation~\cite{USFinalRule2008}, ranking mitigation banking above
in-lieu fee programmes after a decade of evidence that fund-based
mechanisms underperformed ecologically~\cite{HoughHarrington2019}.
A closer institutional precedent is technology carve-outs in
Renewable Portfolio Standards: Kim and Tang~\cite{KimTang2020} show
that mandating minimum purchases from specific energy types
increased technology diversity where general portfolio targets did
not.
...
Mandating exact like-for-like matching for the Trust's purchases reduces
coverage to 12.7\%, confirming that government intermediaries need procurement
flexibility to serve thin markets~\cite{VanTeeffelen2014}.
The rarity multiplier (2$\times$ credits for rare OTGs) has negligible
effect (16.5\%): the bottleneck for underserved types is the
existence of demand, not the quantity per transaction. Bull et
al.~\cite{Bull2017multipliers} showed analytically that uniform
multipliers fail for rare habitats; our results identify why the
failure is empirical too.
...
impact avoidance, and Drechsler~\cite{Drechsler2024} showed formally
that buyer-choice flexibility always reduces ecological benefit. The
NSW data confirm both predictions empirically and identify the
sub-mechanism: relaxing like-for-like at the buyer's discretion
collapses demand into the cheapest types within each formation,
which is exactly the pattern reproduced by the model.
The procurement flexibility mandate does not let individual buyers choose to
substitute across types; instead, regulators direct a fixed 20\% of
compliance demand toward underserved types. The remaining 80\% retains
type-specific like-for-like and the price signal it generates; the redirected
20\% trades type-specific compensation for coverage breadth, a deliberate
efficiency-coverage trade-off rather than a Pareto improvement over
...
patches~\cite{FischerLindenmayer2007}) receive systematically
...
The market is failing on its own terms: designed to compensate for
development impacts on listed threatened communities, it instead
channels investment toward the most commercially accessible types. zu Ermgassen
et al.~\cite{zuErmgassen2020} flagged the latitude on legal grounds:
the 2017 Biodiversity Conservation Act places no legal restriction on
the Trust offsetting using any habitat type anywhere in the state.
The NSW transaction record shows the empirical signature of that
latitude. The Trust's budget (AUD\,47.3M per year) is sufficient to
...
Cost-effectiveness, defined as private outlay per percentage point of
functional coverage gained, is modest under the effective interventions. Bypass
...
A striking asymmetry emerges in the model: coverage is easier to damage
than to improve. The BCT precision mandate reduces coverage by 3.6
percentage points (eliminating a third of functional TEC markets in
simulation), while the best positive intervention (bypass reduction)
gains only 1.2 percentage points. The asymmetry suggests that poorly
designed procurement rules can produce coverage losses that
well-designed interventions, within the range tested, only partially
reverse.
...
analogous coverage biases appear in renewable energy
markets~\cite{ZhouSolomonBrown2024} and water entitlement
markets~\cite{DonnaEspinSanchez2026}, where mandatory or
...
The five interventions evaluated act on demand-side parameters or price;
supplier-side levers (cost-floor adjustments, supply subsidies for rare
types) are out of scope and warrant separate analysis.
...
regulator's stated intent, that every listed ecosystem type should sustain
...
Farmer's market-ecology framework~\cite{Farmer2002} treats financial market
participants as biological species competing for limited profit opportunities,
with population dynamics governed by the profitability of their behavioural
strategies. We build on the formulation of Scholl, Calinescu and
Farmer~\cite{SchollCalinescuFarmer2021}, who showed that the ecological
community matrix of interacting trading strategies can be computed numerically
from a Walrasian agent-based model.
...
identity~\cite{SchollCalinescuFarmer2021}
...
Demand functions follow Brock-Hommes-style CARA agents with heterogeneous
beliefs~\cite{BrockHommes1997}; the closed-form bid rules use Chiarella and
He's specification~\cite{ChiarellaHe2002}. The equivalence of the CARA
derivation and the bid rules below is established in Supplementary Note~1.2
...
\begin{enumerate}
  ...
  \item \textbf{Intermediaries.} Entities appearing on both sides
    ...
    intermediaries.
    CARA demand~\cite{ChiarellaHe2002}:
...
within-formation and across-formation competition for the finite flow of
compliance obligations directly, the dynamic the GLV reduction
characterises, rather than between-strategy capital flows.
...
Calibration validation: (i)~functional coverage among TECs (model: 25.3\%; observed: 22.8\%)
and non-TECs (model: 12.1\%; observed: 15.6\%) emerges without being a calibration target;
(ii)~the formation-level transaction ranking matches observations
(Spearman $\rho = 0.98$, $p < 10^{-10}$);
(iii)~the compliance/BCT obligation split (65.9\%/34.1\%) reproduces
...
\caption{%
  ...
  Cost $\Delta$ = change in direct (open-market) developer expenditure
  relative to Baseline; Fund pay-ins are not included, so scenarios that
  shift obligations into the Fund show a negative Cost~$\Delta$ even when
  total regulatory cost rises.
